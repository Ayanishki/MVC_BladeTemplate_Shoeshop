        ...
        <div class="col-12 grid-margin">
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="form-sample" action="{{ route('product.update', $product->ProID) }}" method="POST"
                                    ...
                                    <div class="col-sm-9">
                                        <select name="catid" class="form-control">
                                            ...
                                            @foreach ($category as $item)
                                                <option value="{{ $item->CatID}}" {{ $item->CatID == $product->CatID ? 'selected' : ''}}>{{ $item->CatName }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        @foreach ($product->variants as $variant)
                            <input type="hidden" name="variants[{{ $loop->index }}][ProVarID]"
                                value="{{ $variant->ProVarID }}">
                            <div class="row">
                                <div class="col-md-2">
                                    <div class="form-group row">
                                        <label class="col-sm-4 col-form-label"
                                            for="variants[{{ $loop->index }}][color_id]">Color</label>
                                        <div class="col-sm-8">
                                            <select class="form-control" id="variants[{{ $loop->index }}][color_id]"
                                                name="variants[{{ $loop->index }}][color_id]">
                                                <option value="0">-- Chọn --</option>
                                                @foreach ($color as $colors)
                                                    <option value="{{ $colors->ColorID }}"
                                                        {{ $variant->ColorID == $colors->ColorID ? 'selected' : '' }}>
                                                        {{ $colors->ColorName }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <div class="col-md-2">
                                    <div class="form-group row">
                                        <label class="col-sm-4 col-form-label"
                                            for="variants[{{ $loop->index }}][size_id]">Size</label>
                                        <div class="col-sm-8">
                                            <select class="form-control" id="variants[{{ $loop->index }}][size_id]"
                                                name="variants[{{ $loop->index }}][size_id]">
                                                <option value="0">-- Chọn --</option>
                                                @foreach ($size as $sizes)
                                                    <option value="{{ $sizes->SizeID }}"
                                                        {{ $variant->SizeID == $sizes->SizeID ? 'selected' : '' }}>
                                                        {{ $sizes->SizeName }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <div class="col-md-3">
                                    <div class="form-group row">
                                        <label class="col-sm-4 col-form-label"
                                            for="variants[{{ $loop->index }}][quantity]">Quantity</label>
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control"
                                                id="variants[{{ $loop->index }}][quantity]"
                                ...
                                <div class="col-md-3">
                                    <div class="form-group row">
                                        <label class="col-sm-4 col-form-label"
                                            for="variants[{{ $loop->index }}][price]">Price</label>
                                        <div class="col-sm-8">

                                            <input type="text" class="form-control"
                                                id="variants[{{ $loop->index }}][price]"
